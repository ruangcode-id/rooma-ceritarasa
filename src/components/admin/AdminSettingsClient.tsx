    ...
    <div className="max-w-4xl pb-20">
      <div className="mb-10">
        <SectionTitle
          ...
          description="Configure daily reservation rules, automatic WhatsApp message templates, and public website optimization."
        />
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
        ...
        <section className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-200">
          <div className="flex items-center gap-3 mb-6 text-slate-900 border-b border-slate-100 pb-4">
            ...
            <div>
              <h2 className="text-lg font-bold">Reservation Rules</h2>
              <p className="text-xs text-slate-500">Booking policy, daily quota, and deposit.</p>
            </div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Max Daily Guests</label>
              ...
              <p className="text-[11px] text-slate-500 mt-2">Total reservation guest limit per day.</p>
            </div>
        ...
        <section className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-200">
          <div className="flex items-center gap-3 mb-6 text-slate-900 border-b border-slate-100 pb-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Website profile appearance when searched on Google.</p>
            </div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="col-span-1 md:col-span-2">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">SEO Title (Title Tag)</label>
              <input 
                type="text" 
                name="seoTitle" 
                value={formData.seoTitle || ""} 
                onChange={handleChange} 
                placeholder="e.g. Rooma Ceritarasa | Premium Fine Dining" 
                className="w-full border-2 border-slate-200 bg-transparent px-4 py-3 text-sm text-slate-900 outline-none transition-all focus:border-slate-900 placeholder:text-slate-300 rounded-xl" 
              />
            </div>
            <div className="col-span-1 md:col-span-2">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">SEO Description (Meta Description)</label>
              <textarea 
                name="seoDescription" 
                rows={3} 
                value={formData.seoDescription || ""} 
                onChange={handleChange} 
                placeholder="Short description that will appear below the title on Google search results..." 
